...
(function($){	  
	$.addFlex = function(t,p) {
			...
			addCellProp: function (){
				$('tbody tr td',g.bDiv).each(function (){
                    ...
                    if (pth!=null){
                        ...
                        $(tdDiv).css({textAlign:pth.align,width: $('div',pth)[0].style.width});
						 //$(tdDiv).css({width: $('div:first',pth)[0].style.width});
						 //$(this).css({"text-align":pth.align});
		...
		//set gDiv
		$(g.gDiv).addClass('flexigrid fg_' + t.id);
		...
		$('thead tr:first th',g.hDiv).each(function (){
			...
			$(this).css({textAlign:this.align, width: this.width + '%'});
		...
		//add row properties
		g.addRowProp();
		if ($(g.bDiv).height() >= $(t).height())
		    $(t).parent().css({ "padding-right": "16px" });
		else
		    $(t).parent().css({ "padding-right": "0px" });
		//set cDrag
